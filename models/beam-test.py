...
class BeamPeller(nn.Module):
    ...
    def __init__(self, vocab_size, max_len, hidden_size, sos_id, eos_id,
                 layer_size=1, rnn_cell='gru', dropout_p=0, use_attention=True, use_beam_search=False):
        super(BeamPeller, self).__init__()
        ...
        if use_teacher_forcing:
            ...
            if self.use_beam_search:
                ...
                for di in range(max_length):
                    predicted_softmax = self.forward_step(speller_input, speller_hidden, listener_outputs, function=function)
                    step_output = predicted_softmax.squeeze(1) # (batch_size, output_size)
                    decode_results.append(step_output)
    ...
    def beam_search_on(self, beam_width=1):
