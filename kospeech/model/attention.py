...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, hidden_dim, num_heads=4):
        ...
        self.W_Q = nn.Linear(hidden_dim, self.dim * num_heads)
        self.W_V = nn.Linear(hidden_dim, self.dim * num_heads)

    def forward(self, query, value, unuse=None):
        batch_size = value.size(0)
        residual = query

        query = self.W_Q(query).view(batch_size, -1, self.num_heads, self.dim)  # BxTxNxD
        value = self.W_V(value).view(batch_size, -1, self.num_heads, self.dim)  # BxTxNxD
...
class LocationAwareAttention(nn.Module):
    ...
    def __init__(self, hidden_dim, smoothing=True):
        ...
        self.W_S = nn.Linear(hidden_dim, 1, bias=True)
    ...
    def forward(self, query, value, prev_attn):
        ...
        score = self.W_S(
            torch.tanh(
                self.W_Q(query.reshape(-1, hidden_dim)).view(batch_size, -1, hidden_dim)
                + self.W_V(value.reshape(-1, hidden_dim)).view(batch_size, -1, hidden_dim)
                + conv_attn
                + self.bias
            )
        ).squeeze(dim=-1)
        ...
        context = torch.cat([context, residual.squeeze(1)], dim=1)
