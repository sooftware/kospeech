...
def build_model(opt, device):
    ...
    listener = build_listener(input_size=opt.n_mels, hstate_size=opt.hstate_size, dropout_p=opt.dropout,
    ...
    speller = build_speller(num_classes=len(char2id), max_len=opt.max_len, sos_id=SOS_token, eos_id=EOS_token,
                            hstate_size=opt.hstate_size << (1 if opt.use_bidirectional else 0),
...
def build_listener(input_size, hstate_size, dropout_p, num_layers, bidirectional, rnn_type, extractor, activation, device):
    """ Various encoder dispatcher function. """
    if not isinstance(input_size, int):
        raise ParameterError("input_size should be inteager type")
    if not isinstance(hstate_size, int):
        raise ParameterError("hstate_size should be inteager type")
    ...
    if hstate_size < 0:
        raise ParameterError("hstate_size should be greater than 0")
    if num_layers < 0:
    ...
    return Listener(input_size=input_size, hstate_size=hstate_size,
...
def build_speller(num_classes, max_len, hstate_size, sos_id, eos_id, attn_mechanism,
    ...
    if hstate_size % num_heads != 0:
        raise ParameterError("{0} % {1} should be zero".format(hstate_size, num_heads))
    if dropout_p < 0.0:
        raise ParameterError("dropout probability should be positive")
    if num_heads < 0:
    ...
    return Speller(num_classes=num_classes, max_length=max_len,
                   hstate_size=hstate_size, sos_id=sos_id, eos_id=eos_id,
