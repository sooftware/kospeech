...
class Speller(nn.Module):
    ...
    def forward(self, inputs, listener_outputs, teacher_forcing_ratio=0.90, use_beam_search=False):
        ...
        if use_beam_search:
            listener_dim = listener_outputs.size(2)
            h_state = _inflate(h_state, self.k, dim=1)

            listener_outputs = _inflate(listener_outputs, self.k, dim=0)
            listener_outputs = listener_outputs.view(self.k, batch_size, -1, listener_dim)
            listener_outputs = listener_outputs.transpose(0, 1)
            listener_outputs = listener_outputs.reshape(batch_size * self.k, -1, listener_dim)
